...
// Delete a terminal session (and its commands via ON DELETE CASCADE)
export const deleteSession = async (req: Request, res: Response) => {
  try {
    ...
    const existing = await query(
      'SELECT session_id FROM terminal_sessions WHERE session_id = $1 AND user_id = $2',
      [sessionId, user.userId]
    );
    ...
    await query(
      'DELETE FROM terminal_sessions WHERE session_id = $1 AND user_id = $2',
      [sessionId, user.userId]
    );

    return res.status(200).json({ message: 'Terminal session deleted' });
  } catch (error) {
    console.error('Delete session error:', error);
    return res.status(500).json({ error: 'Failed to delete terminal session' });
  }
};

export const reopenSession = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as { userId: number; email: string; username: string } | undefined;
    if (!user) {
      return res.status(401).json({ error: 'Authentication required' });
    ...
    await query(
      `UPDATE terminal_sessions
       SET status = 'active', closed_at = NULL, updated_at = CURRENT_TIMESTAMP
       WHERE session_id = $1 AND user_id = $2`,
      [sessionId, user.userId]
    );

    return res.status(200).json({ message: 'Terminal session reopened' });
  } catch (error) {
    console.error('Reopen session error:', error);
    return res.status(500).json({ error: 'Failed to reopen terminal session' });
  }
};
...
export const executeCommand = async (req: Request, res: Response) => {
  try {
    ...
    const normalizedWorkingDir = !workingDir || workingDir === '~'
      ? (process.env.HOME || process.env.USERPROFILE || process.cwd())
      : workingDir;

    const safeWorkingDir = normalizedWorkingDir;
    if (safeWorkingDir.includes('..') || /[<>|&;`$]/.test(safeWorkingDir)) {
      return res.status(403).json({
